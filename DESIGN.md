...
문서 화면은 VitePress의 좌측 사이드바, 중앙 읽기 열, 우측 목차를 유지한다. 데스크톱 사이드바는 희미한 우측 경계로만 분리하고, 표는 읽기 열 너비를 전부 사용한다. 빵부스러기는 본문 제목 앞에서 24px의 간격을 만든다.
...
- **Breadcrumbs:** 13px의 절제된 메타 텍스트이며 현재 위치만 브랜드 보라로 강조한다.
- **Sidebar:** 최상위 항목은 800 굵기이며 데스크톱에서 희미한 세로 경계를 갖는다.
